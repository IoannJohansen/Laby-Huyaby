...
namespace Polska
{
	...
	{
		//DETECKTING ERRORS
		if (lexTable.table[lexPos].lexema == LEX_ID && lexTable.table[lexPos + 1].lexema == LEX_LEFTHESIS && lexTable.table[lexPos + 2].lexema != LEX_ARITHMETIC && lexTable.table[lexPos + 2].lexema != LEX_RIGHTHESIS)return false;	// we dont checking expressions with functions
		...
		for (int i = lexPos; i < lexPos + srcLen; i++)		// expression analyse
		{
			...
			case LEX_SEMICOLON:break;		// bad symbol in expression
